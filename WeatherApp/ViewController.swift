...
extension ViewController: UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        guard let cityName = textField.text, cityName.isEmpty == false else {
                print("Enter a city name")
                return true
        }
        controller.fetchCurrentWeatherData(city: cityName) { (weatherData, error) in
            if let data = weatherData {
                let displayText = "Vreme u \(cityName): \(data.condition), \(data.temperature), \(data.unit.rawValue)"
                self.weatherLabel.text = displayText
            } else {
                self.weatherLabel.text = "Nema podataka za vas grad!"
            }
        }
        return true
    }
